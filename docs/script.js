const translations = {
  en: {
    ...
    "hero.body":
      "RuleMeter tracks source topology, repeated rules, owner-ratified exceptions, and new review deltas across local agent instruction files.",
    "hero.primary": "View on GitHub",
    "hero.secondary": "Try locally",
    "workflow.title": "Track deltas, rewrite never.",
    "workflow.body":
      "RuleMeter does not edit instruction files. It keeps local evidence for cross-file verbatim overlap, same-file repetition, owner decisions, and what changed since the last reviewed run.",
    ...
    "workflow.step4.title": "Review only new deltas",
    "workflow.step4.body": "Use the local state file to separate new, changed, known, and resolved review work.",
    ...
    "signals.queue.body": "Combines open review items into one scoreless queue, then run state separates new work from known items.",
  ...
  ko: {
    ...
    "hero.body":
      "RuleMeter는 로컬 에이전트 지시문 파일의 source topology, 반복 규칙, owner 승인 예외, 새 review delta를 추적합니다.",
    "hero.primary": "GitHub에서 보기",
    "hero.secondary": "로컬에서 실행",
    "workflow.title": "Delta를 추적하고, 파일은 고치지 않습니다.",
    "workflow.body":
      "RuleMeter는 지시문 파일을 수정하지 않습니다. 파일 간 축자 overlap, 같은 파일 내부 반복, owner 결정, 마지막 검토 실행 이후 바뀐 항목을 로컬 증거로 남깁니다.",
    ...
    "workflow.step4.title": "새 delta만 검토",
    "workflow.step4.body": "로컬 state 파일로 new, changed, known, resolved review work를 구분합니다.",
    ...
    "signals.queue.title": "Review queue",
    "signals.queue.body": "열린 검토 항목을 점수 없이 한 queue로 모은 뒤, run state로 새 항목과 이미 아는 항목을 구분합니다.",
  ...
  ja: {
    ...
    "hero.body":
      "RuleMeter は、ローカルのエージェント指示ファイルの source topology、反復規則、owner 承認済み例外、新しい review delta を追跡します。",
    "hero.primary": "GitHub を見る",
    "hero.secondary": "ローカルで試す",
    "workflow.title": "Delta を追跡し、書き換えません。",
    "workflow.body":
      "RuleMeter は指示ファイルを編集しません。ファイル間の逐語 overlap、同一ファイル内の反復、owner decisions、前回レビュー済み run から変わった項目をローカル証拠として残します。",
    ...
    "workflow.step4.title": "新しい delta だけを確認",
    "workflow.step4.body": "ローカル state file で new、changed、known、resolved review work を分けます。",
    ...
    "signals.queue.title": "Review queue",
    "signals.queue.body": "open review items を scoreless queue にまとめ、run state で新しい項目と既知の項目を分けます。",
