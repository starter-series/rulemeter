  ...
  "findings": [
    {
      ...
      "locationText": "/path/to/repo-a/AGENTS.md, /path/to/repo-b/CLAUDE.md"
    }
  ]
}
```
...
```json
{
  "labels": {
    "abc123def4567890": {
      "decision": "actionable",
      "note": "Would review the low-risk duplicate for parity or consolidation."
    }
  }
}
```

Or keep review labels in a separate private file generated by `--label-template`:
...
```json
{
  ...
  "labels": {
    "abc123def4567890": {
      "decision": "actionable",
      "note": "Cross-file overlap is worth reviewing for parity.",
      "kind": "surface_overlap",
      "split": "mixed",
      "splits": ["calibration", "holdout"],
      "signal": "review_duplicate",
      "locations": "/path/to/repo-a/AGENTS.md, /path/to/repo-b/CLAUDE.md"
    }
  }
}
```
...
RuleMeter should be positioned as a non-scoring, read-only duplicate and overlap review aid for agent instruction files. Same-file duplicates are deletion candidates; cross-file surface overlaps are review prompts, not deletion instructions or semantic drift findings. Do not market RuleMeter as an AI safety/security linter, runtime guardrail, agent harness scorecard, semantic prompt optimizer, auto-sync PR bot, or enforcement engine unless the implementation and corpus evidence change substantially.
