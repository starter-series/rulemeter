...
function markdownReport(payload) {
  const lines = [
    ...
    `- same-file duplicate actions: ${payload.metrics.byKind.duplicate}`,
    `- cross-file overlap reviews: ${payload.metrics.byKind.surfaceOverlap}`,
    `- keyword review findings: ${payload.metrics.byKind.risk}`,
    `- keyword review groups: ${payload.metrics.byKind.riskSummary}`,
    `- lexical similarity reviews: ${payload.metrics.byKind.similar}`,
    "",
    "> Non-strict smoke only. Use a private strict corpus with reviewed labels for release evidence.",
    "",
    "## Warnings",
    "",
  ];
