...
`rulemeter` uses `js-tiktoken` by default and tries `o200k_base`, then `cl100k_base`. Pass `--encoding cl100k_base`, `--encoding o200k_base`, or `--model <name>` when you need an explicit tokenizer. If a tokenizer cannot be loaded, output falls back to `fallback_regex` so the result is visibly approximate.
